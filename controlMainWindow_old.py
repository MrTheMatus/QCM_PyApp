...
class ControlMainWindow(QtWidgets.QMainWindow):
    def __init__(self, parent=None, port=None, bd=115200, samples=500):
        ...
        self.batch_timer.timeout.connect(self._batch_save_to_db)
        ...
        self.ui.recordButton.setIcon(QtGui.QIcon(".\\images\\play.png"))  # Set initial icon
    ...
    def toggle_recording(self):
        """
        Toggle recording on and off, updating the button icon accordingly.
        """
        if self.is_recording:
            self.is_recording = False
            self.ui.recordButton.setIcon(QtGui.QIcon(".\\images\\play.png"))  # Update to play icon
            self.worker.reset_buffers(Constants.buffer_samples)  # Reset buffers when stopping recording
            logging.info("Stopped recording.")
        else:
            self.is_recording = True
            self.ui.recordButton.setIcon(QtGui.QIcon(".\\images\\stop-button.png"))  # Update to stop icon
...
def _batch_save_to_db(self, buffer_size_threshold=100):
    """
    Fetch data from Worker and save to the database in batches
    when the buffer size exceeds the threshold.
    """
    if not self.is_recording:
        logging.debug("Recording is off. Skipping batch save.")
        return

    if not self.worker.is_buffer_full(buffer_size_threshold):
        logging.debug("Buffer not full. Skipping batch save.")
        return

    # Fetch and save batch data
    try:
        time_data, signal_data = self.worker.get_batch_data()
        if len(time_data) > 0:
            rows = [
                (timestamp, *[signal[idx] for signal in signal_data])
                for idx, timestamp in enumerate(time_data)
            ]
            self._save_batch_to_db(rows)
    except Exception as e:
        logging.error(f"Error during batch save: {e}")


    def _save_batch_to_db(self, rows):
        """
        Save a batch of rows to the database.
        """
        if not rows:
            logging.debug("No rows to save. Skipping database write.")
            return
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.executemany(
                "INSERT INTO ProcessData (time, signal_1, signal_2, ...) VALUES (?, ?, ?, ...)",
                rows
            )
            conn.commit()
        except sqlite3.Error as e:
            logging.error(f"Database batch save error: {e}")
        finally:
            conn.close()
